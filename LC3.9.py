...
GPIO.setup(9, GPIO.OUT)        #sw_K4_ge(HIGH)
GPIO.setup(10, GPIO.OUT)       #ge_K4_sw(HIGH)
GPIO.setup(11, GPIO.OUT)       #br_K5_rt(HIGH)
GPIO.setup(12, GPIO.OUT)       #bl_K5_ge(HIGH)
...
try:
    ...
    try:                                     #Loop-Auswahl:
        ...
        if data == 3:
            print("-----------------Loop3 inkl. Annemie exkl. Baum-- --")
            GPIO.output(10, GPIO.LOW)          # K4_init
            GPIO.output(10, GPIO.HIGH)         # K4_ON_
            time.sleep(0.1)
            GPIO.output(10, GPIO.LOW)         # K4_OFF
...
try:
    while True:
        if Start:
            ...
            try:
                f = open("/home/pi/NAS/LC.log", "a")
                f.write("\n" + "LC3.9.py started at: " + timestr + "  Loop: " + str(data))
                f.close()
            except:
                print("NAS not mounted, started LC3.9 without NAS")
            Start = False
        try:
        ...
        if t2 == 20:
            ...
            try:
                fobj_out = open("/home/pi/NAS/LC.log",  "a" )
                fobj_out.write("\n" + time.strftime("%Y-%m-%d %H:%M:%S") + "     t: " + str(round(delta,3)) + "--3.9shutdown--" + '\n' )
                fobj_out.close()
            except:
                fobj_out = open("/home/pi/data/LC.log",  "a" )
                fobj_out.write("\n" + time.strftime("%Y-%m-%d %H:%M:%S") + "     t: " + str(round(delta,3)) + "network ERROR!!--3.9 shutdown--" + '\n' )
                fobj_out.close()
