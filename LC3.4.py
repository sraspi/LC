...
try:
    ...
    while True:
        ...
        if t2-t1 == 0:
            ...
            try:
                fobj_out = open(name_log,  "a" )
                fobj_out.write("\n" + time.strftime("%Y-%m-%d %H:%M:%S") + "     t: " + str(round(delta,3)) + "-3.4 shutdown-" + '\n' )
                fobj_out.close()
            except:
                fobj_out = open("/home/pi/data/LC.log",  "a" )
                fobj_out.write("\n" + time.strftime("%Y-%m-%d %H:%M:%S") + "     t: " + str(round(delta,3)) + "network ERROR!!---3.4 shutdown---" + '\n' )
                fobj_out.close()
